...
export function mixinD3(base) {
  ...
  return class extends base {
    ...
    data is an array of {
      ...
      styles?: [[name, value]...]
      events?: [[name, handler]...],
      transitions?: [{duration?: number, attrs?: [...]}...],
      children?: [{tag: string, ...}...],
      selector?: string;
      key?: function;
      ns?: string;
      call?: function | [function, ...params];
    }
    */
    get data() {
      if (!this._data) {
...
function updateAll(changelog, config) {
  const { root, selector, children, key, ns, call, each } = config;
  const changelogs = group(changelog);
  const selection = root
    .selectAll(selector)
    .data(children, key)
    .join((enter) => enter.append((d) => create(d, ns).node()))
...
function setAttrs(s, obj, row, column, value) {
  setField(obj, row, column, value, (k, v) => s.attr(k, v));
}

function setProperties(s, obj, row, column, value) {
  setField(obj, row, column, value, (k, v) => s.property(k, v));
}

function setStyles(s, obj, row, column, value) {
  setField(obj, row, column, value, (k, v) => s.style(k, v));
}

function setEvents(s, obj, row, column, value) {
  setField(obj, row, column, value, (k, v) => s.on(k, v));
}

function setTransitions(s, obj) {
  if (!obj) {
    return;
  }
    ...
    forEach(config, (k, v, ...x) => {
      switch (k) {
        ...
        case "events":
          ...
          break;
        case "tweens":
          forEach(v, (name, func) => s.tween(name, func));
          break;
        case "text":
          setText(s, v);
          break;
    ...
    set(obj, prop, value) {
      ...
      if (Reflect.set(obj, prop, value)) {
        ...
        if (isAtRoot(props, prop)) {
          ...
          // use a light copy to prevent overwritting from immediately settings
